...
export function ConnectionStatus() {
    const { connected, idTab, registered, readiness, comConnected, tagPlaced } = useSelector((s) => s.device);
    return (
        <div style={{ position: 'fixed', bottom: 12, right: 12, padding: '8px 12px', borderRadius: 8, background: '#0b1220', color: '#fff', fontFamily: 'sans-serif', fontSize: 12, opacity: 0.9 }}>
            ...
            <div>COM: {comConnected === null ? '—' : (comConnected ? 'online' : 'offline')}</div>
            <div>tag: {tagPlaced === null ? '—' : (tagPlaced ? 'placed' : 'removed')}</div>
            <div>idTab: {idTab || '—'}</div>
